/**
 * List Contact Notes Tool
 * 
 * Provides functionality to list notes associated with a contact in HubSpot.
...
const inputSchema: InputSchema = {
  ...
  properties: {
    contactId: {
      type: 'string',
      description: 'ID of the contact to list notes for (required)'
    },
    limit: {
      type: 'number',
...
/**
 * List contact notes tool definition
 */
export const tool: ToolDefinition = {
  name: 'listContactNotes',
  description: 'List notes associated with a contact in HubSpot',
  inputSchema,
  handler: async (params) => {
    ...
    try {
      const result = await notesService.listContactNotes(
        params.contactId,
        {
          limit: params.limit,
          after: params.after,
      ...
      throw new BcpError(
        `Failed to list contact notes: ${errorMessage}`,
        'API_ERROR',
