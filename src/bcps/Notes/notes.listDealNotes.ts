/**
 * List Deal Notes Tool
 * 
 * Provides functionality to list notes associated with a deal in HubSpot.
...
/**
 * Input schema for list deal notes tool
 */
const inputSchema: InputSchema = {
  ...
  properties: {
    dealId: {
      type: 'string',
      description: 'ID of the deal to list notes for (required)'
    },
    limit: {
      type: 'number',
...
/**
 * List deal notes tool definition
 */
export const tool: ToolDefinition = {
  name: 'listDealNotes',
  description: 'List notes associated with a deal in HubSpot',
  inputSchema,
  handler: async (params) => {
    ...
    try {
      const result = await notesService.listDealNotes(
        params.dealId,
        {
          limit: params.limit,
          after: params.after,
      ...
      const response = {
        success: true,
        data: result,
        message: 'Successfully retrieved deal notes'
      };

      return enhanceNotesResponse(response, 'listDealNotes', params);
    } catch (error) {
      ...
      throw new BcpError(
        `Failed to list deal notes: ${errorMessage}`,
        'API_ERROR',
